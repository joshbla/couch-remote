...
The canonical installed location is `/Applications/Couch Remote.app`. Keep the installed app running while building, testing, committing, and pushing. Replace and restart it only as the final step. Afterward, verify `accessibility` is `true` in `$TMPDIR/couch-remote-status.json`; do not infer success from controller detection alone.
